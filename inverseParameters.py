...
class InverseParameters:
    ...
    Attributes:
        prior_type (str): Type of prior to use ('random', 'load', 'combination', etc.).
        ...
        regularization_params (Dict[str, Any]): Parameters for regularization terms.
        additional_settings (Dict[str, Any]): Any additional settings required.
    ...
    prior_type: str = 'random'               # 'random', 'load', 'combination', etc.
    prior_filepath: Optional[str] = None     # Filepath to load prior mask
    solver_type: str = 'FISTA'               # 'FISTA', 'ADMM', 'Nesterov', 'PGD'
    max_iter: int = 500                       # Reduced for efficiency
    learning_rate: float = 1e-1              # Increased learning rate
    convergence_threshold: float = 1e-5       # Convergence threshold
    update_method: str = 'set_pixels'        # 'set_pixels', 'gradient', etc.
    epochs: int = 10                          # Number of epochs for iterative solvers
    regularization_params: Dict[str, Any] = field(default_factory=lambda: {
        'tv_weight': 0.1,
        'shape_weight': 0.5
    })
    additional_settings: Dict[str, Any] = field(default_factory=dict)  # Additional settings
    save_interval: int = 50                  # Interval for saving mask evolution
    ...
    nesterov_momentum: float = 0.9          # Nesterov momentum coefficient
    pgd_momentum: float = 0.0               # PGD momentum (if used)
